...
Users can easily create, update, and delete their own secrets using CLI or REST API. Secrets can be created in a project level which is accessible from all of the namespaces in the project, or can just be created in namespace level. These secrets will then can be used as part of the job spec configuration using macros with their names. Only the secrets created at project & namespace the job belongs to can be referenced.
